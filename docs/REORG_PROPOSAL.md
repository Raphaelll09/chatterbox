...
**✅ Sign-off received (see the Resolution column) — this section's flags have been acted on.**

| File(s) | Evidence | Confidence | Resolution |
|---|---|---|---|
| `do_normalize_txt.pl` | Repo's own `apt-packages-pi.txt` comment states outright: "`do_normalize_txt.pl`, the latter currently unused/dead code." No importers, no shell-out call site found anywhere in the Python tree. | **High** — repo self-documents this | ✅ **Deleted in Phase 3** (no sign-off gate needed for this one) |
| `graphify-out/` | Generated cache/report for this AI-assistant's own knowledge-graph tool (80+ AST-cache JSON files, `graph.html`, `manifest.json`) — a build artifact of a dev-time tool, tracked in git. Not consumed by any project code. | **Medium** — gitignore going forward; deleting history is your call | ✅ **Deleted and gitignored** (your call: delete now, not just gitignore going forward) |
| `profile/P4 - First/Second Full try/, profile/Step 7B - 2/, …7B - 3/, …7B - 4/, profile/Step 7C - */, profile/Step 7D - */` | 17 MB of per-run experiment output (CSV/JSONL/XLSX), tracked only because git's `profile/*.csv` gitignore rule is shallow and doesn't reach nested `cadence_NN/` subfolders. | **Low confidence on delete** — this is real research data, not junk; flagging the *location*, not the content | **Your call: move to a separate data/results repo.** Flagged as follow-up work, not executed in this pass — the migration itself (extracting history, standing up a new repo, re-pointing anything that reads these paths) is a separate, deliberate task to schedule, not a drive-by file move. |
| `requirements.txt`, `minimal_requirements.txt` | Both carried their own deprecation banner as the first lines of the file (per `INSTALL.md` and `CLAUDE.md`, deliberately "kept for reference," not accidental cruft). | Not dead by design — kept on purpose, listed here only for completeness | ✅ **Deleted** (your call: fully superseded by `requirements-dev.txt`/`requirements-pi.txt`, recoverable from git history if ever needed for retraining) — `CLAUDE.md`, `INSTALL.md`, `README.md`, `requirements-dev.txt`'s own header comment all updated to stop referencing them as present files |
| `pmic_calibrate.py` vs. `profiling/calibrate.py` (now `tools/measurement/pmic_calibrate.py` vs. `tools/monitoring/profiling/calibrate.py`) | Both write `profile/calibration.json` via different workflows (guided multi-point wizard vs. single-state manual helper). Functional overlap, but not a strict duplicate — worth asking whether one has been superseded by the other in practice. | Needs your call, not a code-level determination | **Not part of Phase 4's sign-off ask** (wasn't in the plan's explicit bring-back list) — still open if you want to revisit it separately. |
