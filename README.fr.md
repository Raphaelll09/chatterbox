...
> ⚠ **Correction (2026-08).** Cette section documentait le caractère `§`. Le code sépare en réalité
> sur `|` (barre verticale) — voir `chatterbox/synth.py`, `sub_utterance_separator = '|'`. `§` est
> traité comme une ponctuation ordinaire et ne découpe rien. Les exemples ci-dessous ont été
> corrigés. À noter également : ce découpage est **spécifique à FastSpeech 2** ; l'utiliser avec un
> modèle Piper provoque une `FileNotFoundError`.
...
Ces quatre colonnes (`pmic_power_w`, `cpu_power_w`, `mem_power_w`, `ext5v_v`) sont dérivées d'un **seul** appel `vcgencmd pmic_read_adc` par tick — le texte est parsé une fois (`research.profiling.parsing.parse_pmic_rails()`), puis chaque colonne en est extraite. `research/profiling/join.py` ajoute `cpu_energy_wh`/`cpu_mean_w` et `mem_energy_wh`/`mem_mean_w` à `per_sentence_results.csv` / `per_stage_results.csv`, à côté des colonnes système (`energy_j`/`energy_wh`, ...) et ampli (`amp_*`) existantes — utile pour voir, par étage du pipeline, si le coût est plutôt CPU ou plutôt mémoire.
...
- `--join` : une fois le benchmark terminé et le profileur arrêté, lance `research/profiling/join.py` pour produire `profile/per_sentence_results.csv` et `profile/per_stage_results.csv`.
- `--export-xlsx` : en plus du join (implicite), exporte vers une feuille Excel prête à coller — voir "Export Excel" ci-dessous.

## Export Excel

`research/benchmark/export_to_xlsx.py` (nécessite `pip install openpyxl`, dépendance optionnelle chargée à la demande) lit `profile/per_sentence_results.csv` / `profile/per_stage_results.csv` et écrit **`profile/exports/chatterbox_paste.xlsx`** — un dossier dédié aux exports, distinct des CSV bruts.

```
python3 do_tts.py --benchmark --play --export-xlsx
# ou, après un --join déjà fait :
python -m research.benchmark.export_to_xlsx [--profile-dir profile] [--out-dir profile/exports]
```
...
## Format de `research/benchmark/sentences_fr.jsonl`
